...
def uppdate_student_input():  
    """Cập nhật danh sách phòng trong Combobox khi thêm/sửa/xóa phòng."""
    conn = connect_db()
    if conn:
        try:
            cur = conn.cursor()
...
openration_menu = tk.Menu(menubar, tearoff=0)
menubar.add_cascade(label="Menu", menu=openration_menu)

openration_menu.add_command(label="1. Quan lu phong", command=lambda: notebook.select(tab_rooms))  
openration_menu.add_command(label="2. Quan ly sinh vien", command=lambda: notebook.select(tab_students))

openration_menu.add_separator()
openration_menu.add_command(label="Thoat", command=root.quit)
...
cbb_room_assign = ttk.Combobox(frame_stu_info, values=[], width=15)
...
def export_students_to_excel():
    """Lấy dữ liệu sinh viên từ DB và xuất ra file Excel."""
    conn = connect_db()
    if conn:
        try:
            filepath = asksaveasfilename(
                ...
                initialfile="DanhSachSinhVien.xlsx"
            )
            if not filepath:
                return
            sql = "SELECT cccd, name, gender, dob, sdt, room_id FROM students ORDER BY room_id, name"
            ...
            df.to_excel(filepath, index=False, sheet_name='SinhVienKTX')
            
            messagebox.showinfo("Thành công", f"Đã xuất dữ liệu thành công ra file:\n{filepath}")
            
        except pd.io.sql.DatabaseError as e:
            messagebox.showerror("Lỗi CSDL", f"Lỗi khi đọc dữ liệu: {e}")
        except Exception as e:
            messagebox.showerror("Lỗi Xuất File", f"Không thể xuất ra file Excel: {e}")
        finally:
            if conn: conn.close()
...
tk.Button(frame_stu_btn, text="Thoát", width=8, command=root.quit).grid(row=0, column=6, padx=5)
tk.Button(frame_stu_btn, text="Xuất Excel", width=10, command=export_students_to_excel).grid(row=0, column=5, padx=5)
